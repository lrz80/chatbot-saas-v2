...
type SentMsg = {
  ...
  contacto: string;
  contenido: string;
  fecha_envio?: string;     // existe siempre en tu SELECT
  sent_at?: string;         // puede no existir si la columna no está creada
  enviado?: boolean;
};
...
export default function FollowUpSettingsPage() {
  ...
  const [mensajesEnviados, setMensajesEnviados] = useState<SentMsg[]>([]);
  ...
  const fetchSettings = async () => {
    try {
      ...
      if (data) {
        const totalMinutos = Number(data.minutos_espera) || 60;
        ...
        setMembresiaActiva(Boolean(data.membresia_activa));
      }
    } catch (error) {
  ...
  const fetchMensajesEnviados = async () => {
    try {
      // por si en el futuro paginas: ?status=sent&page=1&limit=100
  ...
  const handleSave = async () => {
    try {
      setSaving(true);
      const totalMinutos = (Number.isFinite(horasEspera) ? horasEspera : 0) * 60;
    ...
    const fetchUsos = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setUsoFollowup(usos.find((u: any) => u.canal === 'followup') || null);
        }
      } catch (error) {
  ...
  const lista = Array.isArray(mensajesEnviados) ? mensajesEnviados : [];
          ...
          <div className="w-full bg-white/20 h-2 rounded mb-4 overflow-hidden">
            <div
              className={`h-full ${colorBarra(
                calcularPorcentaje(usoFollowup.usados ?? 0, usoFollowup.limite ?? 0)
              )} transition-all duration-500`}
              style={{
                width: `${calcularPorcentaje(usoFollowup.usados ?? 0, usoFollowup.limite ?? 0)}%`,
              }}
            />
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input
              ...
              max={23}
              value={horasEspera}
              onChange={(e) => setHorasEspera(parseInt(e.target.value || '0', 10))}
              onBlur={() => {
      ...
      <section className="bg-gradient-to-r from-purple-800/20 to-fuchsia-600/10 border border-purple-600/30 backdrop-blur-md p-6 rounded-2xl shadow-md space-y-6">
        ...
        <div>
          <label className="block text-sm mb-2 font-semibold text-purple-200">
            Mensaje para clientes que preguntan precios:
          </label>
          <textarea
            value={mensajePrecio}
        ...
        <div>
          <label className="block text-sm mb-2 font-semibold text-purple-200">
            Mensaje para clientes que desean agendar cita:
          </label>
          <textarea
        ...
        <div>
          <label className="block text-sm mb-2 font-semibold text-purple-200">
            Mensaje para clientes que preguntan ubicación:
          </label>
          <textarea
        ...
        {loadingMensajes ? (
          ...
          <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2">
            {lista.map((mensaje, index) => {
              const fechaStr = mensaje.sent_at || mensaje.fecha_envio;
